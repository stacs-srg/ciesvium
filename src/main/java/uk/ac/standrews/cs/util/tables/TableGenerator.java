...
public class TableGenerator {
    ...
    private final List<CSV> data_sets;
    ...
    public TableGenerator(List<String> row_labels, List<CSV> data_sets, PrintStream writer, String table_caption, String first_column_heading, boolean display_output_as_percentages) {
    ...
    public CSV getProcessedData() throws IOException {

        // Get the labels from the data set for the first row - all rows should have the same labels.
        List<String> column_labels = data_sets.get(0).getLabels();
        column_labels.add(0, first_column_heading);

        CSV processed_data = new CSV(column_labels);

        for (int row_number = 0; row_number < data_sets.size(); row_number++) {

            String row_label = row_labels.get(row_number);
            CSV csv = data_sets.get(row_number);

            processed_data.addRecord(getProcessedRow(row_label, csv));
        }

        return processed_data;
    }

    private List<String> getProcessedRow(String row_label, CSV csv) throws IOException {

        List<List<Double>> numerical_values = StatisticValues.parseStrings(csv.getRecords());
